        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    mpPointCloudMappingClient = make_shared<PointCloudMappingClient>(savePCDPath, thprob, thdepth);
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor, mpPointCloudMappingClient);
...
void System::Shutdown()
{
    ...
    mpPointCloudMappingClient->requestFinish();
    // Wait until all thread have effectively stopped
    while(!mpPointCloudMappingClient->isFinished() || !mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA())
    {
        // cout << mpPointCloudMappingClient->isFinished() << " " << mpLocalMapper->isFinished() << " " << mpLoopCloser->isFinished() << " " << mpLoopCloser->isRunningGBA() << endl;
        usleep(5000);
    }
